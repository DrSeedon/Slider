    ...
    public class UI_InfiniteScroll : MonoBehaviour
    {
        ...
        private void SetItems()
        {
            for (int i = 0; i < _scrollRect.content.childCount; i++)
            {
                ...
                _pages.Add(_scrollRect.content.GetChild(i).GetComponent<SliderPage>());
            }
            ...
            /*
            if (_itemCount > 0) 
                Debug.Log($@"Infinite Slider - {gameObject.name}: was set {_itemCount} childs!");
                */
        ...
        private List<SliderPage> _pages = new List<SliderPage>();
        public void SetDefaultSiblings()
        {
            foreach (var p in _pages)
            {
                p.transform.SetSiblingIndex(p.initId);
            }
        }
