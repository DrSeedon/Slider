...
namespace UnityEngine.UI.Extensions
{
    ...
    public class UIVerticalScroller : MonoBehaviour
    {
        ...
        //my variables
        [Header("Elem change params")]
        public float fadeCheckmarkDistance = 150f;
        public float maxY = 500f;
        public float maxDeltaY = 300f;
        public float minCoeffY = 0.4f;
        public float resizeToMinDistance = 250f;

        public bool onScroll = false;
        
        
        float minDistance;
        int oldFocusedElement;
        float fadeCoeff = 0;
        float resizeCoeff = 0;
        ...
        public void Update()
        {
            ...
            if (!onScroll)
            {
                ...
                oldFocusedElement = focusedElementIndex;
                fadeCoeff = 0;
            }
            ...
            for (var i = 0; i < _arrayOfElements.Length; i++)
            {
                ...
                fadeCoeff = Mathf.Clamp(Mathf.InverseLerp(0, fadeCheckmarkDistance, distance[i]), 0, 1);
                resizeCoeff = Mathf.Clamp(Mathf.InverseLerp(0, resizeToMinDistance, distance[i]), 0, 1);
                
                _arrayOfElements[i].GetComponentInChildren<CanvasGroup>().alpha = 1 - fadeCoeff;
                
                _arrayOfElements[i].GetComponent<RectTransform>().sizeDelta = new Vector2(
                    _arrayOfElements[i].GetComponent<RectTransform>().sizeDelta.x, 
                    maxY - maxDeltaY * resizeCoeff);
                ...
                if (minDistance == distance[i] && !onScroll)
                {
                    focusedElementIndex = i;
                    result = _arrayOfElements[i].GetComponentInChildren<TMP_Text>().text;
                }
            }
            ...
            if(Input.touchCount == 0)//|| !UIExtensionsInputManager.GetMouseButton(0))
            {
                // scroll slowly to nearest element when not dragged
                ScrollingElements();
            }
            else if(Input.GetTouch(0).position.y > 900f && Input.GetTouch(0).phase == TouchPhase.Moved)
            {
                onScroll = false;
                ScrollingElements();
            }
        ...
        public float scrollSpeedOnOutput = 2f;
        private void ScrollingElements()
        {
            if(focusedElementIndex == -1) return;
            
            float newY = Mathf.Lerp(scrollingPanel.anchoredPosition.y, scrollingPanel.anchoredPosition.y + distReposition[focusedElementIndex], Time.deltaTime * scrollSpeedOnOutput);
        ...
        {
            onScroll = true;
            focusedElementIndex = element;


            float deltaElementPositionY = -70 + ((maxY - maxDeltaY) * element);//elementSize.rect.height * element;
            Vector2 newPosition = new Vector2(scrollingPanel.anchoredPosition.x, deltaElementPositionY);//scrollingPanel.anchoredPosition.y + distReposition[focusedElementIndex]);
            scrollingPanel.anchoredPosition = newPosition;

        }

        public void ScrollToElement(int element)
        {
            onScroll = true;
            focusedElementIndex = element;
        }
